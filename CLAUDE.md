...
- **Autonomy model**: **per-entity** (`manual` | `auto`) + **system-wide pause** (kill switch). No global Off/Assist/Auto pill — every PO / SKU / vendor carries its own labor switch. The New Request wizard sets it for new POs on **Step 1**; the existing labor switches on Orders / Inventory / Suppliers are the authoritative knobs after creation. Default for new entities is `auto`. The **Pause all agents** kill-switch lives on **Activity & Governance → Agents tab** — when on, every Auto entity is frozen platform-wide; Manual entities are unaffected. **Atlas is never gated** — chat copilot + page-context summaries are always available. **Smart features are never gated** either — autocomplete, category detection, vendor relevance ranking, similar-past-POs insights are UX, not agent actions; they run on Manual entities too. What IS gated by Manual / Auto: agents taking action without user approval (auto-pre-pick, auto-execute below cap, auto-restock on par breach). See `lib/autonomy.ts` for the canonical definitions.
