...
2. ~~**Multi-vendor RFQ Composer** modal~~ — **shipped in Phase 4h** + **4h.2 (tracker + simulated quote ingestion + Award)** + **4h.3 (Award→PO + Bali channel context)**. `RFQComposerModal.tsx` + `RFQTrackerModal.tsx` + `lib/rfqStore.ts` + `lib/poStore.ts`. Composer triggers from the New Request header; tracker sits behind a "Your RFQs" button next to it with a live count badge. Quote arrivals are explicitly labeled "via WhatsApp" or "via email" everywhere they render — reflects Bali reality where vendors are chat/email-first with no portal handshake. Award now: (a) mints a `RuntimePO` at Stage 2 (Quote/Vendor Confirmed) in `poStore` with the channel + AM contact + RFQ id, (b) NewOrdersPage merges runtime POs in front of seeded ORDERS so the same render path works, (c) the right-panel shows a "Quote received via WhatsApp" card sourced from the runtime metadata, (d) the success toast carries a "View in Orders" action button that deep-links via `#order=PO-XXXX`. Still pending: awarded-vendor → directory write-back (vendor composite/triggers bump on award), trigger duplicates on Orders/Suppliers.
...
| # | Flag | What's missing | Fix shape |
|---|---|---|---|
| 1 | 5-step wizard still skips RFQ → quote → negotiate → award flow — **partially shipped** | Step 4 (Review) now surfaces an **Atlas · Quantity Check** card that fuzzy-matches basket items to `finnsSKUs` (by keyword + category), computes `max(burnRate × 3d, par − onHand)` per item, and surfaces per-item suggestions ≥ 20% above the ordered qty. Admin can Accept (updates qty live in basket), Dismiss per item, or Accept all. Card collapses to "All quantities reviewed · Atlas" pill when handled. RFQ/quote/negotiate flow on Step 2 remains unbuilt — Path B (RFQ Composer) exists but the quote negotiation cycle is still mock. Remaining: Bid Comparison & Award as a real negotiation workspace. |
...
| 4 | "Confirm Delivery" no rejection / partial / damaged branch — **largely shipped** | Stage 5 Task Module now has a **3-way match table** (PO Ordered vs Received vs Invoice Qty per line item). Received qty inputs colour-code live (green = match, red = short, amber = excess). QC outcome auto-derives from line item results (all match → pass; mixed → conditional; all fail → fail) but stays overridable. Short shipments auto-surface variance detail in the A-03 dispute draft ("dipesan 24 kg, diterima 20 kg, kurang 4.0 kg"). QC fail seeds dispute draft + auto-opens Source Bridge with Send Dispute / Waive dispute flows. Remaining gaps: per-item condition photo upload, partial payment amount tied to line-item variance, `GoodsReceipt` entity. |
...
| 11 | QC failure alert lacks workflow back to vendor — **partially shipped** | Dispute draft now flows from Orders Stage 5 QC fail → Source Bridge → **Send Dispute via WhatsApp** (A-03 authored, Bahasa Indonesia, editable before send). The Suppliers QC alert card itself still has no actions beyond dismiss. Remaining: **Hold Next PO** gate and **Adjust Score** (gated to Manager) on the Suppliers alert surface. |
...
| 3 | Dispute lifecycle collapsed — **partially shipped** | Dispute cards now have three meaningful resolution actions: **Approve Payment** (closes dispute, logs `dispute-approve`), **Request Credit Note** (inline form — amount + note — logs `dispute-reject` with credit amount in meta), **Escalate to Director** (status → escalated, logs `dispute-escalate`). Actions mutate visible state: card shows resolution badge (✓ Payment approved / ↩ Credit requested / ↑ Escalated) and moves to a Resolved section. `#dispute=PO-XXXX` hash deep-link from Orders switches to Disputes tab and highlights the matching card. `finns-qc-failure` events auto-create a runtime dispute card and switch to the Disputes tab. Remaining gaps: no assignment to a named person, no evidence file upload area, no appeal window, no per-dispute audit trail beyond the action log. |
